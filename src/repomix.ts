...
export async function runRepomix({ exclude = [], diffOnly = false, tokenLimit = 100000 }: RepomixOptions): Promise<RepomixResult> {
    ...
    if (tokenCount > tokenLimit) {
        console.log(`Token count ${tokenCount} exceeds limit ${tokenLimit}, optimizing repomix command...`);
        return await optimizeRepomix({ exclude, diffOnly, tokenLimit, currentTokens: tokenCount, tempFilePath });
    }
    ...
    const directoryMatch = fileContent.match(/<directory_structure>([\s\S]*?)<\/directory_structure>/);

    return {
        filePath: tempFilePath,
        tokenCount,
        directoryStructure: directoryMatch ? directoryMatch[1] : '',
...
Current exclude patterns: ${exclude.join(', ')}
...
`;
    ...
    try {
        ...
        const tokenMatch = output.match(/Estimated token count: (\d+)/);
        const newTokenCount = tokenMatch ? parseInt(tokenMatch[1], 10) : currentTokens;
        ...
        const updatedDirMatch = updatedContent.match(/<directory_structure>([\s\S]*?)<\/directory_structure>/);

        return {
            filePath: tempFilePath,
            tokenCount: newTokenCount,
            directoryStructure: updatedDirMatch ? updatedDirMatch[1] : directoryStructure,
            summary: updatedSummaryMatch ? updatedSummaryMatch[1] : summary
        };
    } catch (error) {
